...
def _validate_result_visibility(
    ...
    for response in result_set.get("responses", []):
        ...
        if response.get("outcome") != "ranked":
            raise KernelError(code, "result response has unsupported outcome")
        ranked_responses += 1
        for item in response.get("items", []):
            ...
            if not required <= set(item):
                raise KernelError(code, "retrieval item hides required authority or explanation fields")
            if not isinstance(item["revision"], int) or item["revision"] < 1:
                raise KernelError(code, "retrieval item does not expose a positive exact revision")
            if not isinstance(item["matched_fields"], list) or not item["matched_fields"]:
                raise KernelError(code, "retrieval item does not expose matched fields")
            if not isinstance(item["explanation"], str) or not item["explanation"].strip():
                raise KernelError(code, "retrieval item does not expose ranking explanation")
            if not isinstance(item["provenance"], list):
                raise KernelError(code, "retrieval item does not expose provenance")
    return {
...
def _validate_pinned_identity(
    ...
    if baseline.get("contract") != expected_contract:
        raise KernelError(code, f"expected pinned contract {expected_contract!r}")
    if baseline.get("mode") != MODE:
        raise KernelError(code, f"pinned evidence mode must be {MODE!r}")
    if baseline.get("live") is not False or baseline.get("repository_mutation") is not False:
        raise KernelError(code, "pinned evidence violates non-live read-only authority")
...
def run_phase3_closure(
    ...
    _validate_pinned_identity(
        lexical_baseline,
        "atlas-phase3-lexical-baseline/0.1",
        "E-PHASE3-LEXICAL",
    )
    _validate_pinned_identity(
        structured_baseline,
    ...
    _validate_pinned_identity(
        fusion_baseline,
        "atlas-phase3-rank-fusion-candidate/0.1",
        "E-PHASE3-FUSION",
    )
    _validate_pinned_identity(
        research_baseline,
        "atlas-phase3-research-foundation-baseline/0.1",
        "E-PHASE3-RESEARCH",
    )
    ...
    lexical_visibility = _validate_result_visibility(lexical_results, "E-PHASE3-LEXICAL-VISIBILITY")
    ...
    _compare_metrics(
        ...
        structured_baseline["metrics"],
        "E-PHASE3-STRUCTURED",
    )
    structured_visibility = _validate_result_visibility(
        structured_results,
        "E-PHASE3-STRUCTURED-VISIBILITY",
    )
    ...
    if (
        fusion_baseline.get("decision") != "rejected"
        or fusion_report.get("recommendation")
        != "reject-candidate-no-quality-gain-over-structured"
    ):
        raise KernelError("E-PHASE3-FUSION", "rejected comparative decision is not preserved")
    fusion_visibility = _validate_result_visibility(fusion_results, "E-PHASE3-FUSION-VISIBILITY")
    ...
    exit_gates = {
        ...
        "specialized_boundaries_pass_policy": (
            lexical_baseline["external_services"] is False
            and lexical_baseline["embeddings"] is False
            and lexical_baseline["vector_database"] is False
            and structured_baseline["external_services"] is False
            and structured_baseline["embeddings"] is False
            and structured_baseline["vector_database"] is False
            and structured_baseline["canonical_body_indexed"] is False
            and fusion_baseline["decision"] == "rejected"
            and fusion_baseline["vector_database"] is False
        ),
...
def validate_completion_report(report: Mapping[str, Any]) -> dict[str, Any]:
    ...
    gates = _require_mapping(
        report.get("exit_gates"),
        "E-PHASE3-COMPLETION-GATES",
        "completion report requires exit gates",
    )
    ...
    boundary = _require_mapping(
        report.get("phase4_entry_boundary"),
        "E-PHASE3-PHASE4-BOUNDARY",
        "completion report requires Phase 4 boundary",
    )
    if (
